...
fn get_packages_from_config(path: &Path) -> io::Result<Vec<String>> {
    let file = File::open(path)?;
    let mut package_list: Vec<String> = io::BufReader::new(file).lines().flatten().collect();
    package_list.sort_unstable();
    Ok(package_list)
}
...
impl SystemConfigSyncronizer for ThreeStateSyncronizer {
    ...
    fn report_up_diff(&self, up_diff: &Self::UpDiff) {
        ...
        println!("{} {}", self.to_mark_explicit_report_msg, up_diff.to_mark_explicit.join(", "));
    }

    fn report_down_diff(&self, down_diff: &Self::DownDiff) {
        println!("{} {}", self.to_remove_report_msg, down_diff.to_remove.join(", "));
        println!("{} {}", self.to_mark_dependency_report_msg, down_diff.to_mark_dependency.join(", "));
    }
...
fn main() -> Result<(), String> {
    ...
    if let Err(e) = pacman_config.sync_up_down(&config_packages) {
        return Err(format!("Error syncronizing: {}", e));
    }
    Ok(())
}
